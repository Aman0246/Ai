...
const register=async(req,res)=>{
    try {
        ...
        if(!username||!email||!password)return res.status(400).send({status:false,message:"Require to fill each field"})
        let user= await User.findOne({email})
        if(user)return res.status(400).send({status:false,message:"User all Ready exist"})
        if(!validator.isEmail(email))return res.status(400).send({status:false,message:"email is invalid"})
...
const login=async(req,res)=>{
    try {
        ...
        if(!email||!password)return res.status(400).send({status:false,message:"Require to fill each field"})
      if(!validator.isEmail(email))return res.status(400).send({status:false,message:"email is invalid"})
      let user= await User.findOne({email})
      if(!user)return res.status(400).send({status:false,message:"invalid User"})
